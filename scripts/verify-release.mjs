...
for (const name of names) {
  ...
  assert.equal(
    ...
    "git+https://github.com/priemskiyyy/silo.git",
  );
  assert.equal(metadata.repository.directory, directory);
  assert.equal(metadata.bugs.url, "https://github.com/priemskiyyy/silo/issues");
  ...
  if (tag !== undefined) {
    // An `Unreleased` heading, or anything else that is not a date, blocks the
    // publish rather than shipping a version nobody dated.
    assert.match(
      entry,
      / - \d{4}-\d{2}-\d{2}$/,
