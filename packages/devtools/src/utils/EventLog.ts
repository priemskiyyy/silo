...
export type RecordedEvent = Omit<SiloDiagnosticEvent, "context"> & {
  id: number;
  context: string;
  summary: string;
...
export class EventLog {
  ...
  add = (event: SiloDiagnosticEvent, showValues: boolean) => {
    this.#next += 1;
    this.#events = [
      { ...event, id: this.#next, ...describeContext(event, showValues) },
      ...this.#events,
    ].slice(0, this.#limit);
    this.#notify();
  };
