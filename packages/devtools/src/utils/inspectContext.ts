/**
 * Copies a context or a stored value into plain, size-bounded data. Reads own
 * property descriptors so getters, toJSON and proxies never run. Redacts
 * sensitive keys always, and hides value-like keys unless values are shown.
 */
export const inspectContext = (context: unknown, showValues: boolean) => {
  ...
  const inspect = (value: unknown, depth: number): unknown => {
    ...
    for (const [key, descriptor] of entries.slice(0, 50)) {
      ...
      if (!showValues && /^(value|raw|data)$/i.test(key)) {
        result[key] = "[Values are hidden]";
        continue;
      }
