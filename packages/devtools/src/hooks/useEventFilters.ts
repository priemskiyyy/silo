...
const NO_FILTERS: EventFilters = { record: null, query: "", kind: null };

/** Record, search and kind filters over the recorded events, with the visible subset derived once. */
export const useEventFilters = (events: Accessor<RecordedEvent[]>) => {
  ...
  const isFiltered = createMemo(() => {
    const { record, query, kind } = filters();

    return record !== null || query.trim() !== "" || kind !== null;
  });
