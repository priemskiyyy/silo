...
import type { SessionStorageAdapterOptions } from "src/types/SessionStorageAdapterOptions";
...
/**
 * Stores JSON text in sessionStorage and observes changes from same-tab frames.
...
export const sessionStorage = ({
  ...
  const writable = (operation: string, key: string) => {
    ...
    if (platform === null) {
      throw new Error(
        `Cannot ${operation} "${key}" through the session-storage adapter: this environment has no sessionStorage, so nothing was persisted.`,
      );
    }
